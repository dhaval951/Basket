...
protocol SkyFloatingTextFieldDelegate
{
    func pickerSelectionForCityFor(StateIndex:Int)
}

@IBDesignable
open class SkyFloatingLabelTextField: UITextField,UIPickerViewDelegate,UIPickerViewDataSource
{
 // swiftlint:disable:this type_body_length
    ...
    //UIDatePicker Variable
    private let datePicker = UIDatePicker()
    let formatter = DateFormatter()
    var indexvalue = 0;

    //NormalPicker Variables
//    var pickerData = [[String : Any]]()
    ...
    //CustomeDelegate Object
    var customeSkyFloatingDelegate: SkyFloatingTextFieldDelegate!
    ...
    //MARK: Left Images setting
    //Left InActive image
    @IBInspectable
    open var leftInActiveImage:UIImage?
    {
        didSet
        {
            //Check leftInActiveImage is there
            if leftInActiveImage != nil
            {
                self.updateLeftImage(leftInActiveImage!)
            }
        }
    }
   
    //Left Active image
    @IBInspectable open var leftActiveImage:UIImage?
    
    //Set left image function
    func updateLeftImage(_ image: UIImage)
    {
        ...
        let view = UIView(frame: CGRect(x: 0, y: 0, width: imageView.frame.size.width + 2, height: imageView.frame.size.height))
        view.addSubview(imageView)
        self.leftView = view
    }
    
    //MARK: Right image function
    //Right InActive image
    @IBInspectable open var rightInActiveImage:UIImage?
    {
        didSet
        {
            //self.updateRightImage(rightInActiveImage!)
            //Check rightInActiveImage is there
            if rightInActiveImage != nil
            {
                self.updateRightImage(rightInActiveImage!)
            }
        }
    ...
    //Right Active image
    @IBInspectable open var rightActiveImage:UIImage?
    ...
    @IBInspectable var maxLength: Int {
        ...
        set {
            ...
            addTarget(
                self,
                action: #selector(limitMaxLength),
                for: UIControlEvents.editingChanged
            )
        }
    }
    func limitMaxLength(textField: UITextField) {
    ...
    @IBInspectable  open var isDatePicker:Bool = false
    {
        didSet
        {
            if isDatePicker
            {
                ...
                self.datePicker.addTarget(self, action: #selector(self.SelectDateFromDatePicker), for: .valueChanged)
            }
        }
    }
    
    //MARK: isNormalPicker
    @IBInspectable  open var isNormalPicker:Bool = false
    {
        didSet
        {
            if isNormalPicker
    ...
    //MARK: Date Selection method
    
    @objc private func SelectDateFromDatePicker()
    {
        self.text = formateDateFromDatePicker(date: datePicker.date)
    }
    
    //
    private func formateDateFromDatePicker(date:Date) -> String
    {
//        formatter.dateFormat = "YYYY-MM-dd"
    ...
    private func StringToDate(strDate:String) -> Date
    {
//        formatter.dateFormat = "YYYY-MM-dd"
        formatter.dateFormat = "MM-dd-yyyy"
...
//    }
    ...
    */
    ...
    override open func becomeFirstResponder() -> Bool
    {
        ...
        //Check leftActiveImage is there
        if leftActiveImage != nil
        {
          self.updateLeftImage(leftActiveImage!)
        }
        //Check rightActiveImage is there
        if rightActiveImage != nil
        {
            self.updateRightImage(rightActiveImage!)
        }
        //Check isDatePicker
        if isDatePicker
        {
            ...
            {
                self.datePicker.setDate(StringToDate(strDate: self.text!), animated: true)
            }
            self.text = formateDateFromDatePicker(date: datePicker.date)
        }
        
        //Check for normal picker
        if isNormalPicker
        {
//            if (self.text?.isEmpty())!
//            {
                //When empty select first row
              //   self.text = GetSetID_NameForPicker(atIndex: 0)
            //     self.pickerView.selectRow(0, inComponent: 0, animated: true)
//            }
//            else
//            {
//                //Selected row in pickerview itself
//                pickerView.selectRow(GetSelectedIndexForNoarmalPicker(), inComponent: 0, animated: true)
//            }
    ...
    */
    open func titleLabelRectForBounds(_ bounds: CGRect, editing: Bool) -> CGRect {
        if editing {
            //dhaval changes
            return CGRect(x: 0 + 3, y: 0, width: bounds.size.width, height: titleHeight())
        }
    ...
    //MARK: UIPickerDatasource method
...
//        if self.tag == 1
//        {
//            return ((pickerData[row])["areas"].arrayValue)["name"].stringValue
//            
//        }else
//        {
...
//        }
    ...
    public func pickerView(_ pickerView: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String?
    {
            ...
            //List of Paty list
//            return (pickerData[row] as! Party).partyName
        
        if self.tag == 1
        {
//            return ((pickerData[row].array["areas"][row])["name"].stringValue
            return(pickerData[indexvalue]["areas"][row])["name"].stringValue

        }
        else{
            return (pickerData[row])["name"].stringValue
        }
    ...
    {
            self.text = GetSetID_NameForPicker(atIndex: row)
    }
//
//    //MARK: SetTitle For PickerView
    func GetSetID_NameForPicker(atIndex:Int) -> String
    {

        //List of Paty list
        //            self.accessibilityHint = String((pickerData[atIndex])["id"])
        if self.tag == 1
        {
            return(pickerData[indexvalue]["areas"][atIndex])["name"].stringValue
        }
        else
        {
            indexvalue = atIndex;
            return (pickerData[atIndex])["name"].stringValue
        }

    }
    //
//    //MARK: GetSelected Index for NormalPickerView
//    public func GetSelectedIndexForNoarmalPicker() -> Int
//    {
// 
////            if let localParty:[JSON] = pickerData as? [JSON] {
////                return localParty.index(where: { $0.id == Int64(self.accessibilityHint!)})!
////            }
//        
//        return 0
//    }
